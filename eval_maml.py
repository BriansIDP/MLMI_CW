...
def calc_accuracy(eval_output, labels):
    correct = torch.sum(eval_output == labels)
    return correct
...
###############################################################################
if args.model == 'TDNN':
    baseline = XVector(args.nfea, args.nhid, args.nframeout, args.nsegout, args.nlabels)
elif args.model == 'LSTM':
    baseline = LSTMemb(args.nfea, args.nhid, args.nsegout, args.nlayers, args.nlabels)
if args.cuda:
    baseline.cuda()
...
def evaluate(data, model):
    ...
    optimiser = torch.optim.SGD(model.parameters(), lr=args.lr_a)
    task_list = []
    eval_task_list = []
    last_id = 0
    ntasks = nspeaker // args.nlabels
    model_state_dict = model.state_dict()
    for i in range(ntasks):
        fast_task, meta_task, last_id = data.get_eval_task_subset(args.nlabels, args.npoints, last_id)
        task_list.append(fast_task)
        eval_task_list.append(meta_task)
    for i, task in enumerate(task_list):
        ...
        # Do meta forwarding
        meta_task = eval_task_list[i]
        # Arrange the current batch
        features, labels = list(zip(*meta_task))
        features_in = torch.cat(features, 0).to(device)
        labels = torch.tensor(labels).to(device)
        eval_output = model(features_in)
        eval_output = eval_output.argmax(dim=-1)
        correct = calc_accuracy(eval_output, labels)
        print(correct.item())
...
try:
    ...
    baseline_acc = evaluate(val_loader, baseline)
    logging('lr_a {:2.5f} | baseline accuracy {:2.2f}\% | maml accuracy {:2.2f}\%'.format(args.lr_a, baseline_acc*100, accuracy*100))
# try:
...
#             logging('| {:5d}/{:5d} iterations | lr {:02.2f} | ms/iteration {:5.2f} | '
#                 'loss {:5.2f}'.format(
